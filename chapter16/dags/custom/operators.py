...
class GlueTriggerCrawlerOperator(BaseOperator):
    ...
    def execute(self, context):
        hook = AwsBaseHook(self._aws_conn_id, client_type="glue", region_name=self._region_name)
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
            raise RuntimeError("An error occurred while triggering the crawler: %r" % response)
